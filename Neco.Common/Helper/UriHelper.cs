...
public static class UriHelper {
	...
	private static readonly IReadOnlySet<String> _superfluousQueryParams = new[] {
	...
	}.Union(_thirdPartyQueryParameters.Where(p => p.Length > 1)).ToHashSet();
	...
	public static String NormalizeUri(Uri uri) {
		...
		if (uri.IsAbsoluteUri) {
			...
			host = String.Empty;
			Uri absUri = new(_exampleBaseUri, uri);
		...
		IEnumerable<String> queryParams = queryCollection
			...
			.Order()
			.WhereNotNull()
			.Where(key => !_superfluousQueryParams.Contains(key, StringComparer.OrdinalIgnoreCase))
