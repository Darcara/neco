...
public class BaseContextTests {
	...
	public void HandlesFeaturesCorrectly() {
		...
		Assert.That(ctx.Features, Has.Count.EqualTo(1));
		Assert.That(ctx.Features["Key"], Is.EqualTo("Data"));
		...
		Assert.That(ctx.Features, Has.Count.EqualTo(2));
		Assert.That(ctx.Features[typeof(String)], Is.EqualTo("KeyAndData"));
